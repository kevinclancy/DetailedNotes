...
Any arrow $(h;v)$ factoring $(f,v;u)$ through $(\pi_1,u)$ must have $h;\pi_1 = f$
and $h;\pi_2 = \psi;v$.   
...
its uniqueness. It commutes, since $z;\pi_2 = \psi;v$. It also preserves points, since
...
A morphism of $\mathbf{SignVar}$ from $(\Sigma, X)$ to $(\Delta, Y)$ is then a pair of a $\mathbf{Sign}$ morphism and a $\mathit{Fam}(\mathbf{Sets})$ morphism sharing the same base morphism, i.e. a triple $$(u, (f_{(\sigma_1, \ldots, \sigma_n, \sigma)})_{(\sigma_1, \ldots, \sigma_n, \sigma) \in S^\star \times S}, (a_\sigma)_{\sigma \in S} )$$ 
where $$u  : S \to T$$ and $$f_{(\sigma_1, \ldots, \sigma_n, \sigma)} : \mathcal F_{(\sigma_1, \ldots, \sigma_n, \sigma)} \to \mathcal G_{(u(\sigma_1), \ldots, u(\sigma_n), u(\sigma))}$$
and $$a_\sigma : X_\sigma \to Y_{u(\sigma)}$$
Given a signature-with-variables $(\Sigma, (X_\sigma)_{\sigma \in |\Sigma|})$ and a set model $(\Delta, (Y_\tau)_{\tau \in |\Delta|}, \sem{-})$ we must find a bijection
\begin{prooftree}
\AxiomC{$((\Sigma, (\mathcal M_\sigma)_{\sigma \in |\Sigma|}, [-]) \defeq \mathit{TermModel}(X)) \overset{\mbf{S}-\mbf{Model}}{\longrightarrow} (\Delta, (Y_\tau)_{\tau \in |\Delta|}, \sem{-})$}
\doubleLine
\UnaryInfC{$(\Sigma, (X_\sigma)_{\sigma \in |\Sigma|}) \overset{\mbf{SignVar}}{\longrightarrow} (\Delta, (Y_\tau)_{\tau \in | \Delta |}) $}
\end{prooftree}
From bottom to top, we map $$(u, (f_{(\sigma_1, \ldots, \sigma_n, \sigma)})_{(\sigma_1, \ldots, \sigma_n, \sigma) \in S^\star \times S}, (a_\sigma)_{\sigma \in S} )$$ to 
$$((u, (f_{(\sigma_1, \ldots, \sigma_n, \sigma)})_{(\sigma_1, \ldots, \sigma_n, \sigma) \in S^\star \times S}), (\sem{-}_{a}^{\sigma})_{\sigma \in |\Delta|} )$$
where
$$(\sem{-}_a^{\sigma} : \mathit{Terms}_\sigma(X) \to Y_{u(\sigma)})$$
is given by\\~\\
\begin{tabular}{lll}
$\sem{x}_a^\sigma$ &  $\defeq$ & $a_\sigma(x)$ \\
$\sem{F(M_1, \ldots, M_n)}_a^\sigma$ & $\defeq$ & $\sem{f_{(\sigma_1, \ldots, \sigma_n, \sigma)}(F)}(\sem{M_1}_a^{\sigma_1}, \ldots, \sem{M_n}_a^{\sigma_n})$
\end{tabular}\\~\\~\\
The following diagram must commute: 

\[\begin{tikzcd}
	{\mathcal M_{\sigma_1} \times \cdots \times \mathcal M_{\sigma_n}} &&& {Y_{u(\sigma_1)} \times \cdots \times Y_{u(\sigma_n)}} \\
	\\
	{\mathcal M_\sigma} &&& {Y_{u(\sigma)}}
	\arrow["{[F]}"', from=1-1, to=3-1]
	\arrow["{\sem{f_{(\sigma_1, \ldots, \sigma_n, \sigma)}(F)}}", from=1-4, to=3-4]
	\arrow["{\sem{-}^\sigma_a}"', from=3-1, to=3-4]
	\arrow["{\sem{-}^{\sigma_1}_a \times \cdots \times \sem{-}^{\sigma_n}_a}", from=1-1, to=1-4]
\end{tikzcd}\]
Along the bottom-left corner we have $$(M_1,\ldots,M_n) \mapsto F(M_1,\ldots,M_n) \mapsto \sem{f_{(\sigma_1, \ldots, \sigma_n, \sigma)}(F)}(\sem{M_1}_a^{\sigma_1}, \ldots, \sem{M_n}_a^{\sigma_n})$$
Along the top-right corner we have $$(M_1,\ldots,M_n) \mapsto (\sem{M_1}_a^{\sigma_1}, \ldots, \sem{M_n}_a^{\sigma_n}) \mapsto \sem{f_{(\sigma_1, \ldots, \sigma_n, \sigma)}(F)}(\sem{M_1}_a^{\sigma_1}, \ldots, \sem{M_n}_a^{\sigma_n})$$
...
\begin{center}
\begin{tikzcd}
X \times J \ar[drr, "f^\sharp \times J" below] & \ar[drr,"\pi^*(f^\sharp)~~~" left] \ar[rr,"\gamma_{\psi}^{-1};\Lambda^{-1}(f^\sharp;e)"] \ar[l,"\gamma_{\psi}^{-1}" above] \pi^*(X) & & Y \\
 & & \Pi(Y) \times J \ar[r,"\gamma_{\Pi(\varphi)}" below] & \pi^*(\Pi(Y)) \ar[u, "\gamma_{\Pi(\varphi)}^{-1}; \Lambda^{-1}(e)" right] 
\end{tikzcd}
\end{center}
...
\begin{center}
\begin{tikzcd}
u^* \Pi \ar[d, "\eta u^* \Pi" right] \\
\Pi \pi^* u^* \Pi \ar[d, "\Pi \alpha \Pi" right] \\
\Pi (u \times \mathit{id})^* \pi^* \Pi \ar[d, "\Pi (u \times \mathit{id}^*)\pi^* \Pi" right] \\
\Pi (u \times \mathit{id})^* 
\end{tikzcd}
\end{center}
...
\begin{center}
\begin{tikzcd}[sep=70.0]
Z  \ar[rrd, "f = \langle f_1 \text{,} f_2 \rangle"] \ar[ddr, "g = \langle \langle g_1 \text{,} g_2 \rangle \text{,} g_3 \rangle~~~~~~~~~~~~~~~~" below] \ar[dr, "~\langle g_1 \text{,} g_3 \rangle" right, dashed] & & \\
& I \times X \ar[r, "\langle \pi;u\text{,} \pi' \rangle"] \ar[d,"\langle id\text{,} \pi' \rangle" left] & 
  \ar[d,"\langle id \text{,} \pi' \rangle"] J \times X \\
 & (I \times X) \times X \ar[r, "\langle \pi;\langle \pi;u \text{,} \pi' \rangle \text{,} \pi' \rangle" below] & (J \times X) \times X 
\end{tikzcd}
...
%We construct the following sequence of arrows.
%$$\delta^*(X) \overset{\red{(\eta_Y)}_{\delta^*(X)}}{\longrightarrow} (Y \Rightarrow (\delta^*(X) \times Y))$$
%$$X \overset{\red{(\eta_Y)}_{\delta^*(X)}^{\blu{\sharp}}}{\longrightarrow} \mathit{Eq}(Y \Rightarrow (\delta^*(X) \times Y))$$
%$$X \overset{\red{(\eta_Y)}_{\delta^*(X)}^{\blu{\sharp}}}{\longrightarrow} \mathit{Eq}(Y \Rightarrow (\delta^*(X) \times Y)) \overset{((\langle \mathit{Eq}(\pi), \mathit{Eq}(\pi') \rangle^{\circ}; \mathit{ev})^{\red{\sharp}})^{\circ}}{\longrightarrow} \mathit{Eq}(Y) \Rightarrow \mathit{Eq}(\delta^*(X) \times Y)$$
%%Finally, transposing gives
%$$X \times \mathit{Eq}(Y) \overset{(\red{(\eta_Y)}_{\delta^*(X)}^{\blu{\sharp}};((\langle \mathit{Eq}(\pi), \mathit{Eq}(\pi') \rangle^{\circ}; \mathit{ev})^{\red{\sharp}})^{\circ})^{\red{\sharp}} }{\longrightarrow} \mathit{Eq}(\delta^*(X) \times Y)$$
%%We can expand the transpositions to obtain a more algebraically tractable form
%$$\blu{\eta} $$
%%Now we must show that this is the inverse of the canonical transformation

%The aforementioned canonical map arises as the transpose of 
%$$\delta^*(X) \times Y \overset{\delta^*(X) \times (\blu{\eta_{I,J}})_Y}{\longrightarrow} \delta^*(X) \times \delta^*Eq(Y) 
% \overset{\langle \delta^*(\pi), \delta^*(\pi') \rangle^\circ}{\longrightarrow} \delta^*(X \times Eq(Y)) $$



%which is in correspondence with
...
\begin{center}
\begin{tikzcd}[sep=80]
(I \times X, X) \ar[d,"(\pi \text{,} \pi')" left] \ar[r, "(\langle \pi;u \text{,} f \rangle \text{,} \langle \pi;\pi \text{,} \pi' \rangle;f)"] & \ar[d,"(\pi \text{,} \pi')"] (J \times Y , Y) \\ 
(I,X) \ar[r,"(u\text{,}f)" below] & (J,Y)
\end{tikzcd}
\end{center}
...
\begin{mathpar}
\inferrule
  {E \doteq \sem{\Gamma \vdash \sigma : \mathit{Type}}}
  {\sem{\Gamma, x:\sigma \vdash x : \sigma} = \mathit{TODO}}
\end{mathpar}
...
\inferrule
  ...
  {\sem{\Gamma, \Delta[M/x] \vdash N[M/x] : \tau[M/x]} = \mathit{TODO}}
\end{mathpar}
From now on, let's not transcribe into latex unless I have already figured out the interpretation...

\begin{mathpar}
\inferrule[Contraction]
  {\sem{\Gamma,x:\sigma,y:\sigma,\Delta \vdash \tau : \mathit{Type}} = X}
  {\sem{\Gamma,x:\sigma,\Delta[x/y] \vdash \tau[x/y] : \mathit{Type}} \doteq (p\delta_{(\Gamma,\sigma)})^*(X)}
\and
\inferrule[Weakening]
  {\sem{\Gamma \vdash \sigma : \mathit{Type}} = X \\ \sem{\Gamma \vdash \tau : \mathit{Type}} = Y}
  {\sem{\Gamma,x:\sigma \vdash \tau : \mathit{Type}} \doteq (p\epsilon_X)^*(Y)}
\end{mathpar}
