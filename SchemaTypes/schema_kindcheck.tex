...
Lines 7-10, use the record type constructor to define a subschema representing credit cards. It denotes the set of instances which map the list [``billingAddr''] to any string and the list [``cardType''] to any string satisfying the \lstinline[mathescape]{$\blu{CardType}$} predicate. 
...
Our index langauge is a fragment of the calculus of constructions, with the unnotable addition of strings. Its interpretation appears, for example, in \cite{Hofmann97syntaxand}, and is displayed in Figure \ref{fig:index-interp} for convenience. 
