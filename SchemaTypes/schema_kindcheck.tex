...
$\sem{\blum{\Omega} \vdash \forall \blum{q}. \kappa}_\omega \defeq \{ \fm{\sem{\blum{\Omega, a : q} \vdash \kappa}_{(\omega, x)} }{x \in \sem{\blum{\Omega \vdash q}}_\omega} \}$\\
\hspace{2.0ex} $= \Pi_{\blum{\Omega},\blum{q}} \sem{\blum{\Omega, a : q} \vdash \kappa}$\\~\\
...
For semantic contexts $\Omega$ and sorts $X \in \mathit{St}(\Omega)$, we define the right adjoint $\Pi_{X}$ of $\frak p(X)^* : \mathit{Fam}(\mbf{Sets})_{\Omega} \to \mathit{Fam}(\mbf{Sets})_{\Omega . X}$ as

\begin{tabular}{l}
$\Pi_{X}(\fm{B_{(\omega, x)}}{(\omega,x) \in \Omega . X}) \defeq \fm{\Pi_{x \in X_\omega} B_{(\omega,x)}}{\omega \in \Omega}$\\~\\
$\Pi_{X} \fm{f_{(\omega,x)} : B_{(\omega,x)} \to C_{(\omega,x)}}{(\omega,x) \in \Omega .X} \defeq$\\ 
\hspace{2.0ex} $\fm{\Pi_{x \in X_\omega} \hspace{2.0ex} f_{(\omega,x)}}{\omega \in \Omega}$
\end{tabular}\\~\\~\\
...
\end{prooftree}~\\~\\
From top to bottom, this bijection, which we'll call $\phi$, takes a morphism $$\fm{\langle f_{(\omega,x)} : A_\omega \to B_{(\omega,x)} \rangle_{x \in X_\omega}}{\omega \in \Omega}$$ to $$\fm{f_{(\omega,x)} : A_\omega \to B_{(\omega,x)}}{(\omega,x) \in \Omega.X}$$
From bottom to top, $\phi$'s inverse $\phi^\circ$ takes a morphism $$(f_{(\omega,x)} : A_\omega \to B_{(\omega,x)})_{(\omega,x) \in \Omega . X}$$ to $$\fm{\Pi_{x \in X_\omega} f_{(\omega,x)}}{\omega \in \Omega}$$ 

$\phi$ underlies an adjunction $\frak p(X)^* \dashv \Pi_{\Omega, X}$. To prove this, first consider a morphism $g$, where $$g = \fm{g_{\omega}}{(\omega) \in \Omega} : \fm{C_{\omega}}{\omega \in \Omega} \to \fm{A_\omega}{\omega \in \Omega}$$
and a morphism $f$ where
...
$= \fm{(\Pi_{x \in X_{\omega}}~~~f_{(\omega,x)}) \circ g_\omega}{\omega \in \Omega}$\\
$= \phi^\circ(f) \circ g$\\~\\
Next, consider a morphism $f$, where
...
$= g \circ \phi(f)$\\~\\
To obtain the counit of this bijection at component $\fm{B_{(\omega, x)}}{(\omega,x) \in \Omega.X}$, writing $\pi_x$ for the projection $\Pi_{x \in X_\omega} B_{(x,\omega)} \to B_{(x,\omega)}$, we map the identity morphism $$\fm{\langle \pi_x \rangle_{x \in X_\omega}}{\omega \in \Omega} : \Pi_X \fm{B_{(\omega,x)}}{(\omega,x) \in \Omega} \to \Pi_X \fm{B_{(\omega,x)}}{(\omega,x) \in \Omega}$$ through $\phi$, obtaining the counit $\epsilon$ as $$\fm{\pi_{(\omega,x')} : \Pi_{x \in X_\omega} B_{(\omega,x)} \to B_{(\omega,x')}}{(\omega,x') \in \Omega . X}$$ It is a morphism of type
...
Transposing gives 
$$\fm{\Pi_{x' \in X_{u(\psi)}}~\pi_{(u(\psi),x')}}{\psi \in \Psi} $$

TODO: Demonstrate that the type of this. Show it is an inverse.

Since $\mathit{Fam}(\mbf{Sets})$ is a split fibration, we have $$\frak q(u,X)^* \frak p(X)^* = (\frak p(X) \circ \frak q(u,X))^* = \frak p(X \{ u \})^* u^*$$
