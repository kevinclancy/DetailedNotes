...
% TODO: explain weakening and the + notation here. maybe also explain overline notation
...
% TODO: the + notation in the above definition is not defined
...
\end{prooftree}~\\~\\
From top to bottom, this bijection, which we'll call $\phi$, takes a arrow $$\fm{\langle f_{(\omega,x)} : A_\omega \to B_{(\omega,x)} \rangle_{x \in X_\omega}}{\omega \in \Omega}$$ to $$\fm{f_{(\omega,x)} : A_\omega \to B_{(\omega,x)}}{(\omega,x) \in \Omega.X}$$
From bottom to top, $\phi$'s inverse $\phi^\circ$ takes a arrow $$(f_{(\omega,x)} : A_\omega \to B_{(\omega,x)})_{(\omega,x) \in \Omega . X}$$ to $$\fm{\Pi_{x \in X_\omega} f_{(\omega,x)}}{\omega \in \Omega}$$ 
