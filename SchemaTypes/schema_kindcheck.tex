...
\end{prooftree}
From top to bottom, $(-)^\sharp$ takes an arrow $$\fm{\langle f_{(\omega,x)} : A_\omega \to B_{(\omega,x)} \rangle_{x \in X_\omega}}{\omega \in \Omega}$$ to $$\fm{f_{(\omega,x)} : A_\omega \to B_{(\omega,x)}}{(\omega,x) \in \Omega.X}$$
From bottom to top, $(-)^\flat$ takes an arrow $$(f_{(\omega,x)} : A_\omega \to B_{(\omega,x)})_{(\omega,x) \in \Omega . X}$$ to $$\fm{\Pi_{x \in X_\omega} f_{(\omega,x)}}{\omega \in \Omega}$$ 
...
%$= (\langle \mbf{T}(\blum{\Omega};\blum{p};\blum{\Psi};\blum{j}) \circ \frak p(\sem{\blum{q}} \{ \mbf{T}(\blum{\Omega};\blum{p};\blum{\Psi};\blum{j}) \}), \frak v_{\sem{\blum{q}}  \{ \mbf{T}(\blum{\Omega};\blum{p};\blum{\Psi};\blum{j}) \}} \rangle_{\sem{\blum{q}}})^* \sem{\blum{\Omega, a : p, \Psi, b : q} \mid \Gamma \vdash \tau' : \kappa'})^\sharp$\\
