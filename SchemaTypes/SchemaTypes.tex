...
\inferrule[Path]
  ...
  {\sem{\Sigma \mid \Gamma@(\{ (S_1,\ldots,S_n).[\phi] \}) \vdash (S_1,\ldots,S_n).[\phi] :: \mathit{KString}(\mathit{Prefix}~\epsilon,\phi)}_{\sigma}(\gamma,a) \defeq \\ \{ s \in \mathit{Strings} \mid a([f_{1 \sigma}(\gamma),\ldots,f_{n \sigma}(\gamma),s]) \text{ is defined} \}}
\and
\inferrule[StrAbs]
