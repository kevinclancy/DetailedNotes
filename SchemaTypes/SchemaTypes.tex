...
\begin{lemma}
If $\Sigma,\zeta,\zeta' \vdash \kappa$ then $\Sigma,\zeta',\zeta \vdash \kappa$ and $$\sem{\Sigma,\zeta,\zeta' \vdash \kappa} = \langle \langle \pi;\pi, \pi' \rangle, \pi;\pi' \rangle^* \sem{\Sigma,\zeta',\zeta \vdash \kappa}$$
\label{lemma:kwf-exchng}
\end{lemma}
 
\begin{proof}
By induction on the structure of $\Sigma,\zeta,\zeta' \vdash \kappa$.
For brevity, we define $f \defeq \langle \langle \pi;\pi, \pi' \rangle, \pi;\pi' \rangle$ in what follows.
\begin{description}
\item[KWF-Str]:~\\
Apply the exchange lemma to the premise $\Sigma,\zeta,\zeta' \vdash d$ to get $\Sigma,\zeta'\zeta \vdash d$ 
and $\sem{\Sigma,\zeta,\zeta' \vdash d} = f^* \sem{\Sigma,\zeta'\zeta \vdash d}$. Apply \begin{sc}KWF-Str\end{sc} to get $$\Sigma,\zeta',\zeta \vdash \mathit{KString}(d,\phi)$$
Furthermore,we have\\~\\
$\sem{\Sigma,\zeta,\zeta' \vdash \mathit{KString}(d,\phi)}_\sigma$\\
$= (~(\mathcal P (X), \subseteq)~),~~\small{X \defeq \{ \{ \epsilon \mapsto s\} \in \mathit{Inst} \mid s \in \sem{\Sigma,\zeta,\zeta' \vdash d}_\sigma \wedge \mathit{chars}(s) \subseteq \phi \}}$\\
$= (~(\mathcal P (X), \subseteq)~),~~\small{X \defeq \{ \{ \epsilon \mapsto s\} \in \mathit{Inst} \mid s \in (f^*\sem{\Sigma,\zeta',\zeta \vdash d})_\sigma \wedge \mathit{chars}(s) \subseteq \phi \}}$\\
$= (~(\mathcal P (X), \subseteq)~),~~\small{X \defeq \{ \{ \epsilon \mapsto s\} \in \mathit{Inst} \mid s \in \sem{\Sigma,\zeta',\zeta \vdash d}_{f(\sigma)} \wedge \mathit{chars}(s) \subseteq \phi \}}$\\
$= \sem{\Sigma,\zeta',\zeta \vdash \mathit{KString}(d,\phi)}_{f(\sigma)}$\\
$= (f^* \sem{\Sigma,\zeta',\zeta \vdash \mathit{KString}(d,\phi)})_\sigma $
\item[KWF-Tree]:~\\
