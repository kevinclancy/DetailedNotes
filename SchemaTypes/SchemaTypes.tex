...
Furthermore, substituting $\langle \mathit{id}, \sem{\Sigma \vdash S} \rangle$ for $f$ in the Beck-Chevalley condition for simple products, we have\\~\\
%$(\langle \mathit{id}, \sem{\Sigma \vdash S} \rangle^* \sem{\Sigma,\zeta \vdash \forall \zeta'. \kappa})_\sigma$\\~\\
%$= \sem{\Sigma,\zeta \vdash \forall \zeta'. \kappa}_{\langle \mathit{id}, \sem{\Sigma \vdash S} \rangle(\sigma)}$\\~\\
%$= (\Pi_{\sem{\Sigma,\zeta}, \sem{\zeta'}} \sem{\Sigma,\zeta,\zeta' \vdash \kappa})_{\langle \mathit{id}, \sem{\Sigma \vdash S} \rangle(\sigma)}$
$(\langle \mathit{id}, \sem{\Sigma \vdash S} \rangle^* \sem{\Sigma,\zeta \vdash \forall \zeta'. \kappa})_\sigma$\\~\\
$= (\langle \mathit{id}, \sem{\Sigma \vdash S} \rangle^* \Pi_{\sem{\Sigma,\zeta},\sem{\zeta'}} \sem{\Sigma,\zeta,\zeta' \vdash \kappa})_\sigma$\\~\\
$\cong (\Pi_{\sem{\Sigma},\sem{\zeta'}} (\langle \mathit{id}, \sem{\Sigma \vdash S} \rangle \times \mathit{id})^* \sem{\Sigma, \zeta, \zeta' \vdash \kappa})_\sigma$\\~\\
