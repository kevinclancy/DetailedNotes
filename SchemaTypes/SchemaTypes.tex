...
$$\langle \mathit{id}, \sem{\Sigma \vdash S} \rangle^* \sem{\Sigma, \zeta \vdash A} = \sem{\Sigma \vdash A[S/\zeta]}$$ %TODO: make use of this fact; I need to revise the interpretation of wf coeffects to use the notation \sem{\Sigma \vdash S_i}_\sigma instead of \sigma S_i
\end{lemma}
...
$= \sdisp{\bigcap_{(S_1,\ldots,S_n).[\phi] \in A} \{ f \in \mathit{Inst} \mid \forall s \in \mathit{keys}(f \! \mid_{\rho S_1, \ldots, \rho S_n}).~\mathit{chars}(s) \subseteq \phi \},~\leq}$\\~\\
$= \sdisp{\bigcap_{(S_1,\ldots,S_n).[\phi] \in A} \{ f \in \mathit{Inst} \mid \forall s \in \mathit{keys}(f \! \mid_{\sigma S_1[S/\zeta], \ldots, \sigma S_n[S/\zeta]}).~\mathit{chars}(s) \subseteq \phi \},~\leq}$
$= \sdisp{\bigcap_{(S_1,\ldots,S_n).[\phi] \in A[S/\zeta]} \{ f \in \mathit{Inst} \mid \forall s \in \mathit{keys}(f \! \mid_{\sigma S_1, \ldots, \sigma S_n}).~\mathit{chars}(s) \subseteq \phi \},~\leq}$\\~\\
$= \sem{\Sigma \vdash A[S/\zeta]}_\sigma$
\end{proof}

\begin{lemma}
If $\Sigma,\zeta \vdash \kappa$ and $\Sigma \vdash S$ then $\Sigma \vdash \kappa[S/\zeta]$ and $$\langle \mathit{id}, \sem{\Sigma \vdash S} \rangle^* \sem{\Sigma,\zeta \vdash \kappa} = \sem{\Sigma \vdash \kappa[S/\zeta]}$$ 
\end{lemma}

\begin{proof}
By induction on the proofs of $\Sigma,\zeta \vdash \kappa$ and $\Sigma,\zeta \vdash A$. 
